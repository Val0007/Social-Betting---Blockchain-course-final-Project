        ...
        <p className="text-sm mb-2">Total Pool: {Web3.utils.fromWei(prediction.totalPool, 'ether')} ETH</p>        <div className="flex gap-2 mb-2 flex-col">
          {prediction.options.map((option, i) => (
            prediction.myBet.amount > 0n && prediction.myBet.index == i ? (
              ...
              <div key={i}>
                <span className="px-2 py-1 bg-emerald-950 text-white text-lg rounded-sm cursor-pointer" 
                  onClick={() => {
            ...
            <div><button className='bg-amber-300 px-5 py-2 mt-2 cursor-not-allowed text-lg'>Settled</button></div> :
            <div>
                <button className='bg-amber-300 px-5 py-2 mt-2 cursor-not-allowed text-lg'>BET IS ACTIVE</button>
            </div>
